...
public class RiskController {
    ...
    public Result<HedgeRiskIndex> calculateHedgeRiskIndex(
            @RequestBody(required = false) List<StockQuote> quotes) {
        log.info("计算对冲风险指数, 请求体是否带数据: {}", (quotes != null ? "是(" + quotes.size() + "只)" : "否"));
        HedgeRiskIndex riskIndex;
        if (quotes == null || quotes.isEmpty()) {
            riskIndex = riskCalculationService.calculateHedgeRiskIndex();
        } else {
            riskIndex = riskCalculationService.calculateHedgeRiskIndex(quotes);
        }
        return Result.success(riskIndex);
    }
    ...
    public Result<RiskAnalysisReport> generateRiskReport(
            @RequestBody(required = false) List<StockQuote> quotes) {
        log.info("生成风险分析报告, 请求体是否带数据: {}", (quotes != null ? "是(" + quotes.size() + "只)" : "否"));
        RiskAnalysisReport report;
        if (quotes == null || quotes.isEmpty()) {
            report = riskCalculationService.generateRiskReport();
        } else {
            report = riskCalculationService.generateRiskReport(quotes);
        }
        return Result.success(report);
    }
}
